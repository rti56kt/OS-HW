...
int main(int argc, char *argv[]){
    ...
    while(remain_process){
        if(wait_queue.empty() == false){
            for(list<int>::iterator it = wait_queue.begin(); it != wait_queue.end(); it++){
                waiting_time[*it]++;
            }
        }
        for(int i = 0; i < total_process; i++){
            if(arrival_time[i] == time_cnt){
                if(wait_queue.empty() == true){
                    ...
                    for(list<int>::iterator it = wait_queue.begin(); ; it++){
                        if(remain_time[i] < remain_time[*it]){
        ...
        if(who_is_busy != -1){
            ...
            if(remain_time[who_is_busy] == 0){
                ...
                if(wait_queue.empty() == false){
                    if(remain_time[wait_queue.front()] < remain_time[who_is_busy]){
                        for(list<int>::iterator it = wait_queue.begin(); ; it++){
                            if(remain_time[who_is_busy] < remain_time[*it]){
                                wait_queue.insert(it, who_is_busy);
                                break;
                            }
                            if(it == wait_queue.end()){
                                wait_queue.push_back(who_is_busy);
                                break;
                            }
                        }
